...
describe('KycService', () => {
  ...
  describe('initiateKyc', () => {
    it('throws if profile not in KYC_PENDING', async () => {
      jest.spyOn(prisma.userProfile, 'findUnique').mockResolvedValue({ ownerState: OwnerState.PROFILE_PENDING } as any);
      await expect(service.initiateKyc('u1', {})).rejects.toThrow(BadRequestException);
    });

    it('creates KYC case if valid', async () => {
      jest.spyOn(prisma.userProfile, 'findUnique').mockResolvedValue({ ownerState: OwnerState.KYC_PENDING } as any);
      const createSpy = jest.spyOn(prisma.ownerKycCase, 'create').mockResolvedValue({} as any);
  ...
  describe('handleWebhook', () => {
    it('throws on invalid signature', async () => {
      ...
      await expect(service.handleWebhook('bad_sig', {})).rejects.toThrow(/Invalid webhook signature/);
    });
    ...
    it('returns early if idempotency check fails (status not PENDING)', async () => {
      ...
      jest.spyOn(prisma.ownerKycCase, 'findUnique').mockResolvedValue({ status: VerificationStatus.VERIFIED } as any);
    ...
    it('updates correctly on success and masks data', async () => {
      jest.spyOn(provider, 'verifyCallbackSignature').mockReturnValue(true);
      jest.spyOn(prisma.ownerKycCase, 'findUnique').mockResolvedValue({ id: 'case1', userId: 'u1', status: VerificationStatus.PENDING } as any);
      const updateKycSpy = jest.spyOn(prisma.ownerKycCase, 'update').mockResolvedValue({} as any);
      const updateProfileSpy = jest.spyOn(prisma.userProfile, 'update').mockResolvedValue({} as any);
      ...
      expect(updateKycSpy).toHaveBeenCalledWith(expect.objectContaining({
        where: { id: 'case1' },
    ...
    it('updates to NEEDS_REVIEW on name mismatch', async () => {
      ...
      await service.handleWebhook('sig', {
        reference: 'ref1',
        status: 'SUCCESS',
        data: { rawAadhaar: '123412341234', name: 'John Doe (Mismatch)', dob: '1990' },
      });
