  ...
  describe('processPropertyDocument', () => {
    it('creates a risk signal on exact match', async () => {
      jest.spyOn(prisma.propertyDocument, 'findUnique').mockResolvedValue({
        id: 'doc1', checksum: '12345', propertyId: 'prop1',
      } as any);

      jest.spyOn(prisma.propertyDocument, 'findMany').mockResolvedValue([
        { id: 'doc2', checksum: '12345', propertyId: 'prop2' } as any
      ]);

      jest.spyOn(prisma.riskSignal, 'findFirst').mockResolvedValue(null);
      const createSpy = jest.spyOn(prisma.riskSignal, 'create').mockResolvedValue({} as any);

      await processor.process({ data: { entityType: 'PROPERTY_DOCUMENT', entityId: 'doc1' } } as any);
  ...
  describe('processListingMedia', () => {
    it('creates near-duplicate signal based on pHash threshold', async () => {
      jest.spyOn(prisma.listingMedia, 'findUnique').mockResolvedValue({
        id: 'med1', sha256Hash: 'a', pHash: '11110000', listingId: 'list1',
      } as any);
      ...
      await processor.process({ data: { entityType: 'LISTING_MEDIA', entityId: 'med1' } } as any);
    ...
    it('ignores pHashes beyond distance threshold', async () => {
      jest.spyOn(prisma.listingMedia, 'findUnique').mockResolvedValue({
        id: 'med1', sha256Hash: 'a', pHash: '1111111111111111', listingId: 'list1',
      } as any);
