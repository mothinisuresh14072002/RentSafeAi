...
export class SandboxPaymentProvider implements PaymentProvider {
  private readonly secret =
    process.env.PAYMENT_WEBHOOK_SECRET || 'rentsafe-sandbox-webhook-secret';
  async createOrder(input: {
    ...
    return {
      providerOrderId: `sandbox_${input.idempotencyKey}`,
      amount: input.amount,
...
export class RazorpayTestPaymentProvider extends SandboxPaymentProvider {
  async createOrder(input: {
    ...
    return {
      ...order,
      providerOrderId: `razorpay_test_${input.idempotencyKey}`,
      sandbox: true,
    };
  }
}
