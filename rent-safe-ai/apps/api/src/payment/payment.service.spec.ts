...
describe('PaymentService safety guarantees', () => {
  const payload = {
    eventId: 'evt-1',
    providerOrderId: 'sandbox-key-1',
    status: PaymentStatus.AUTHORIZED,
  };
  const signature = createHmac('sha256', 'rentsafe-sandbox-webhook-secret')
  ...
  it('rejects a forged webhook signature', async () => {
    ...
    await expect(
      service.handleWebhook('forged', payload),
  ...
  it('does not duplicate a replayed provider event', async () => {
    const tx = {
      paymentOrder: {
        findUnique: jest
          .fn()
          .mockResolvedValue({ id: 'order-1', status: PaymentStatus.CREATED }),
      },
      paymentWebhookEvent: {
    ...
    await expect(service.handleWebhook(signature, payload)).resolves.toEqual({
      replay: true,
    });
  ...
  it('returns the already-created order when a concurrent idempotency insert loses the race', async () => {
    ...
    const tx = {
      paymentOrder: { create: jest.fn().mockRejectedValue({ code: 'P2002' }) },
    };
    const prisma = {
      paymentOrder: {
        findUnique: jest
    ...
    const eligibility = {
      assertEligible: jest
        .fn()
        .mockResolvedValue({ eligible: true, items: [] }),
    };
    const service = new PaymentService(
      prisma as any,
      eligibility as any,
  ...
  it('does not create an order when eligibility is lost', async () => {
    ...
    const eligibility = {
      assertEligible: jest.fn().mockRejectedValue(
        new BadRequestException({
          code: 'PAYMENT_NOT_ELIGIBLE',
          items: [{ code: 'NO_HARD_FRAUD_HOLD', passed: false }],
        }),
      ),
    };
    const service = new PaymentService(
