...
export class FraudReportsService {
  ...
  async create(reporterId: string, dto: CreateFraudReportDto) {
    if (
      ...
      !['LISTING', 'USER', 'MESSAGE'].includes(dto.subjectType)
    ) {
      throw new BadRequestException('Unsupported report category or subject');
    }
    if (
      !dto.narrative ||
      dto.narrative.trim().length < 20 ||
      dto.narrative.length > 5000
    ) {
      throw new BadRequestException(
        'Narrative must be between 20 and 5000 characters',
      );
    }
    ...
    if (count >= maxPerDay)
      throw new ForbiddenException('Daily report limit reached');
    const duplicate = await this.prisma.fraudReport.findFirst({
      where: {
        reporterId,
    ...
    if (duplicate)
      throw new BadRequestException(
        'You already have an active report for this subject',
      );

    const ownerId = await this.resolveOwner(dto.subjectType, dto.subjectId);
    const severity = HIGH_RISK.has(dto.category)
      ? SignalSeverity.HIGH
      : SignalSeverity.MEDIUM;
    const report = await this.prisma.$transaction(async (tx) => {
      const created = await tx.fraudReport.create({
        data: {
          reporterId,
          subjectType: dto.subjectType,
          subjectId: dto.subjectId,
      ...
      if (
        ...
        process.env.FRAUD_HIGH_SEVERITY_PAYMENT_HOLD !== 'false'
      ) {
        await tx.paymentHold.create({
          data: {
            ownerId,
            reportId: created.id,
            reason: 'High-severity tenant fraud report pending review',
          },
        });
      }
      if (ownerId && process.env.FRAUD_NOTIFY_OWNER !== 'false') {
        await tx.outboxEvent.create({
          data: {
            ...
            payload: {
              userId: ownerId,
              title: 'Safety review opened',
              body: 'We received a safety report related to one of your listings. Please review your account and avoid requesting payment outside RentSafe.',
              eventType: 'FRAUD_HOLD_OPENED',
              deduplicationKey: `fraud:${created.id}:owner`,
  ...
  async decide(
    id: string,
    reviewerId: string,
    action: 'ASSIGN' | 'INVESTIGATE' | 'RESOLVE' | 'DISMISS' | 'RELEASE_HOLD',
    resolution?: string,
  ) {
    const report = await this.prisma.fraudReport.findUnique({
      where: { id },
      include: { paymentHolds: true },
    });
    if (!report) throw new NotFoundException('Fraud report not found');
    if (action !== 'ASSIGN' && action !== 'INVESTIGATE' && !resolution?.trim())
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (
        ...
        await tx.paymentHold.updateMany({
          where: { reportId: id, status: 'PENDING_REVIEW' },
          data: {
            status: 'RELEASED',
            releasedAt: new Date(),
