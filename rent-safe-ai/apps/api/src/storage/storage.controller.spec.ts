...
describe('StorageController', () => {
      ...
      providers: [
        {
          ...
          useValue: {
            generatePresignedUploadUrl: jest
              .fn()
              .mockResolvedValue('http://upload.url'),
  ...
  describe('requestUploadUrl', () => {
    it('throws if unauthorized owner tries to upload', async () => {
      jest
        .spyOn(policies, 'canAccessPropertyEvidence')
        .mockRejectedValue(new ForbiddenException());
      await expect(
        controller.requestUploadUrl({ user: { userId: 'bad_user' } } as any, {
          propertyId: 'p1',
          targetType: 'DOCUMENT',
          extension: 'pdf',
    ...
    it('rejects spoofed or invalid mimes for documents', async () => {
      await expect(
        controller.requestUploadUrl({ user: { userId: 'u1' } } as any, {
          propertyId: 'p1',
          targetType: 'DOCUMENT',
          extension: 'exe',
    ...
    it('returns signed URL for valid document', async () => {
      const res = await controller.requestUploadUrl(
  ...
  describe('getMediaUrl', () => {
    it('throws if file is quarantined or infected', async () => {
      ...
      await expect(
        controller.getMediaUrl(
          { user: { userId: 'u1', roles: ['OWNER'] } } as any,
          'd1',
        ),
    ...
    it('returns download url if file is CLEARED', async () => {
      jest.spyOn(prisma.propertyDocument, 'findUnique').mockResolvedValue({
        id: 'd1',
        propertyId: 'p1',
        quarantineStatus: QuarantineStatus.CLEARED,
        objectKey: 'k1',
      } as any);

      const res = await controller.getMediaUrl(
        { user: { userId: 'u1', roles: ['OWNER'] } } as any,
        'd1',
      );
